...
; We're using 64-bit system calls and registers
...
; Above we're using hex '0xA' to represent a newline but you can also use base-10 - '10'
; Any characters with (double)quotes will be concatenated into a singl string. Example: "Foo" is the same as "F", "o","o"
; This is unchanged from 32-bit, as the string is the same.
; Strings don't need to be null terminated 
    ; UNLESS you plan on using 'printf' instead of a syscall. This is because 'printf' is in the c library and the printf function expects the null operator (\0) to know when to finish printing a string
    ; In assembly, the numbr 0 can be used to represent the null operator or its hex form 0x0
...
; a trick to get the length of a string. It looks like a pointer subtraction, $ being the pointer to the current instruction, ie the end of the string, and message being the pointer to the beginning of the string
